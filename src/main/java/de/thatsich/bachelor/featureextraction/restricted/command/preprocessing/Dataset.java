...
/**
 * <p>
 * Java implementation of a class for reading in and accessing numerical data to
 * be used by neural networks.
 * </p>
 * 
 * <p>
 * The <code>Dataset</code> class reads in data from a flat file. The flat file
 * is seen as a set of data "elements." An element is equivalent to a line of
 * data in the file that contains an input vector of size m and, optionally, an
 * output vector of size n, is space-delimited and is terminated with the
 * newline character ('\n'):
 * </p>
 * 
 * <p>
...
public class Dataset
{
	 ...
	 * 
	 * @param fileName
	 *            The name of the file (full or relative path)
	 */
	public void readFile( String fileName )
	{
			...
			BufferedReader reader = new BufferedReader( new FileReader( fileName ) ) )
		{
			...
			for ( int line = 0; line < numberOfElements_; line++ )
			{
				...
				{
					elements_[line].getInputVector()[i] = ( new Double( tokenizer.nextToken() ) ).doubleValue();
				}

				// read in the output vector (if any)
				for ( int i = 0; i < outputSize_; i++ )
				{
					elements_[line].getOutputVector()[i] = ( new Double( tokenizer.nextToken() ) ).doubleValue();
				}
			}
		}
		catch ( IOException e )
