...
abstract class FeedforwardNetwork
{
	...
	public void test( Dataset testingSet )
	{
		...
		// output accuracy
		System.out.println( "Accuracy = " + ( ( new Double( numberCorrect ).doubleValue() ) / ( new Double( numberOfTestingPairs ).doubleValue() ) ) * 100.00 + "%" );
	}
	...
	protected boolean feedForward( DatasetElement vectorPair )
	{
		...
		for ( int layerIndex = 0; layerIndex < this.hiddenLayer_.size(); layerIndex++ )
		{
			...
			for ( int hidden = 0; hidden < hiddenLayer.getSize(); hidden++ )
			{
				...
				for ( int input = 0; input < inputSize; input++ )
				{
					weightedSum += inputLayer_.getNeuron( input ).getActivation() * synapseToCurrentLayer.getSynapse( input, hidden ).getWeight() + hiddenNeuron.getBias();
				}
		...
		for ( int output = 0; output < outputSize; output++ )
		{
			...
			{
				weightedSum += lastHiddenLayer.getNeuron( hidden ).getActivation() * lastSynapse.getSynapse( hidden, output ).getWeight() + outputNeuron.getBias();
			}
			...
			double outputError = Math.abs( outputVector[output] - outputLayer_.getNeuron( output ).getActivation() );
