...
class RPropNetwork extends FeedforwardNetwork
{
     ...
     */
    public RPropNetwork(int inputSize, int hiddenSize, int outputSize)
    {
        super(inputSize, hiddenSize, outputSize);

        // initialize input-to-hidden update-value matrix
        inputToHiddenUpdateValues_ =
          new RPropUpdateValue[inputSize][hiddenSize];
        for (int input = 0; input < inputSize; input++){
            inputToHiddenUpdateValues_[input] =
              new RPropUpdateValue[hiddenSize];
            for (int hidden = 0; hidden < hiddenSize; hidden++){
                inputToHiddenUpdateValues_[input][hidden] =
                    new RPropUpdateValue();
            }
        }

        // initialize hidden-to-output update-value matrix
        hiddenToOutputUpdateValues_ =
          new RPropUpdateValue[hiddenSize][outputSize];
        for (int hidden = 0; hidden < hiddenSize; hidden++){
            hiddenToOutputUpdateValues_[hidden] =
              new RPropUpdateValue[outputSize];
            for (int output = 0; output < outputSize; output++){
                hiddenToOutputUpdateValues_[hidden][output] =
                  new RPropUpdateValue();
            }
        }
    }
                ...
                for (int output = 0; output < outputSize; output++){
                    ...
                    for (int hidden = 0; hidden < hiddenSize; hidden++){
                        hiddenToOutputSynapses_.getSynapse(hidden, output).
                          addError((outputVector[output] - outputActivation) *
                          outputActivation *
                ...
                for (int hidden = 0; hidden < hiddenSize; hidden++){
                    ...
                    for (int input = 0; input < inputSize; input++){
                        inputToHiddenSynapses_.getSynapse(input, hidden).
                          addError(hiddenActivation * (1.0 - hiddenActivation) *
                          errorFromOutputLayer *
     ...
     * @param updateValues The matrix of update-values corresponding to
     *   the synaptic weights
     */
    private void updateWeights(SynapticMatrix synapses, 
      RPropUpdateValue[][] updateValues)
    {           
        ...
        for (int column = 0; column < numberOfColumns; column++){
            for (int row = 0; row < numberOfRows; row++){
                ...
                double product = synapse.getError() *
                  synapse.getPreviousError();
