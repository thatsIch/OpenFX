...
public class NetworkPrediction implements INetworkPrediction {
    private static final double PREDICTION_TRUE_THRESHOLD = 0.75;
    ...
    private void calcTrueFalsePositiveNegative(IError[][] onlyError, Double[][] predictions) {
        ...
        for (int col = 0; col < cols; col++) {
            for (int row = 0; row < rows; row++) {
                ...
                final double prediction = predictions[col][row];
                final boolean isPositive = prediction > PREDICTION_TRUE_THRESHOLD;
    ...
    @Override
    public ReadOnlyObjectProperty<String[][]> errorClasses() {
        return this.errorClasses.getReadOnlyProperty();
    }

    @Override
    public ReadOnlyObjectProperty<Double[][]> errorPredictions() {
        return this.errorPredictionMat.getReadOnlyProperty();
    }
