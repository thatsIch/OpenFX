...
public class Images
{
	...
	public static Mat[][] split(Mat image, int frameWidth, int frameHeight)
	{
		...
		if (frameHeight == 0)
		{
			throw new IllegalArgumentException("FrameHeight is 0.");
		}
		if (frameWidth == 0)
		{
			throw new IllegalArgumentException("FrameHeight is 0.");
	...
	public static Mat getHistogram(Mat image)
	{
		...
		{
			throw new IllegalArgumentException("Image is not grayscale. " + image.type());
		}
		if (image.empty())
		{
		...
		for (int i = 0; i < bins; i++)
		{
			...
			Core.line(canvas, new Point(i, canvas.rows()), new Point(i, canvas.rows() - (histogram[i] * canvas.rows() / peak)), new Scalar(200));
		}
